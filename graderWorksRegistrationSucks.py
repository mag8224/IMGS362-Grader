...
def align_images(image, template):
	...
	# piece of paper
	thresh = cv2.threshold(imageGray, 0, 255,
						   cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]

	thresh_template = cv2.threshold(templateGray, 0, 255,
						   cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]
	...
	cnts_temp = cv2.findContours(thresh_template.copy(), cv2.RETR_EXTERNAL,
							cv2.CHAIN_APPROX_SIMPLE)
	cnts = imutils.grab_contours(cnts)
	cnts_temp = imutils.grab_contours(cnts_temp)

	questionCnts = []
	questionCnts_template = []
	...
	for c in cnts_temp:
		...
		# have an aspect ratio approximately equal to 1
		if w >= 30 and h >= 30 and ar >= 0.9 and ar <= 1.1:
			questionCnts_template.append(c)

	cv2.drawContours(image, questionCnts, -1, (0, 0, 255), 1)
	cv2.drawContours(template, questionCnts_template, -1, (0, 0, 255), 1)
	...
	for i in range(0, 1612):
		for j in range(0, 1245):
			dc_im = image[i, j]
			dc_temp = image[i, j]
			# print(dc_im)
			if dc_im[0] == all(c_red):
				coord = (i,j)
				ptsA.append(coord)
			if dc_temp.all() == all(c_red):
				coord = (i,j)
				ptsB.append(coord)
...
# Scale image and template to fit on screen
...
aligned_show = imutils.resize(aligned, width=700)
template_show = imutils.resize(template, width=700)
...
image = aligned[780:1400, 100:1100]
